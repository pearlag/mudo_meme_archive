    ...
    try {
      ...
      if (memesData && Array.isArray(memesData) && memesData.length > 0) {
        ...
        const dbMemes: Meme[] = memesData.map((dbMeme: any) => ({
          ...
          likes: dbMeme.likes,
          isFavorite: true, // 좋아요 페이지이므로 모두 좋아요 상태
        ...
        const likedMemesFiltered = allMemes.filter(meme => {
          if (typeof window !== 'undefined') {
            const likedMemeIds = JSON.parse(localStorage.getItem('likedMemes') || '[]') as string[];
  ...
  return (
    ...
    >
      <div className="space-y-6">
        ...
        <Card>
          <CardHeader>
            <CardTitle>좋아요한 짤 ({filteredMemes.length}개)</CardTitle>
          </CardHeader>
          <CardContent>
              <div className="pb-4">
              ...
              <div className="text-center py-20">
                ...
                <p className="text-xl text-muted-foreground">좋아요한 짤이 없습니다</p>
                <p className="text-sm text-muted-foreground mt-2">
                  마음에 드는 짤에 좋아요를 눌러보세요
                </p>
              </div>
            )}
          </CardContent>
